...
export function EducationLevelsSection({ rows }: { rows: Row[] }) {
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await createEducationLevel(newName, Number(newOrder) || 0);
        toast.success("เพิ่มวุฒิการศึกษาแล้ว");
        setNewName("");
        setNewOrder("");
        router.refresh();
      } catch (err) {
  ...
  function saveEdit() {
    ...
    startTransition(async () => {
      try {
        await updateEducationLevel(id, editName, Number(editOrder) || 0);
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <div className="md:col-span-1 border border-border rounded-lg p-4 bg-card h-fit">
        <h3 className="font-semibold mb-4 text-sm">เพิ่มวุฒิการศึกษา</h3>
        <form onSubmit={handleAdd} className="space-y-3">
          <div className="space-y-1.5">
            <Label htmlFor="newEdu" className="text-xs">ชื่อวุฒิ *</Label>
            <Input
              id="newEdu"
              value={newName}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="newEduOrder" className="text-xs">ลำดับ (ตัวเลข, 0 = บนสุด)</Label>
            <Input
              id="newEduOrder"
              type="number"
              value={newOrder}
              ...
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={3} className="h-24 text-center text-muted-foreground text-sm">
                    {search ? "ไม่พบข้อมูลที่ตรงกับเงื่อนไข" : "ยังไม่มีวุฒิการศึกษา"}
                  </TableCell>
                </TableRow>
              ) : (
                filtered.map((row) => {
      ...
      <ConfirmDialog
        ...
        title="ยืนยันการลบวุฒิการศึกษา"
        description={`ต้องการลบ "${deleteRow?.name ?? ""}" ใช่หรือไม่? ระบบจะปฏิเสธหากมีพนักงานใช้วุฒินี้อยู่`}
        confirmLabel="ลบ"
        variant="destructive"
