...
export function DepartmentsSection({ rows }: { rows: DeptRow[] }) {
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <div className="md:col-span-1 border border-border rounded-lg p-4 bg-card h-fit">
        <h3 className="font-semibold mb-4 text-sm">เพิ่มหน่วยงาน</h3>
        <form onSubmit={handleAdd} className="space-y-3">
          <div className="space-y-1.5">
            <Label htmlFor="newDept" className="text-xs">ชื่อหน่วยงาน *</Label>
            <Input
              id="newDept"
              value={newName}
              ...
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={2} className="h-24 text-center text-muted-foreground text-sm">
                    {search ? "ไม่พบข้อมูลที่ตรงกับเงื่อนไข" : "ยังไม่มีหน่วยงาน — เพิ่มได้ที่ด้านซ้าย"}
                  </TableCell>
                </TableRow>
              ) : (
                filtered.map((row) => {
      ...
      <ConfirmDialog
        ...
        description={`ต้องการลบ "${deleteRow?.name ?? ""}" ใช่หรือไม่? ระบบจะปฏิเสธหากมีพนักงาน/ตำแหน่งใช้งานอยู่`}
        confirmLabel="ลบ"
        variant="destructive"
