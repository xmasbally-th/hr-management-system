...
import { exportTravelRequests } from "@/lib/actions/export-actions";
...
import { TravelRequestTable, type TravelRequestRow } from "@/components/travel-request-table";
import { TravelBudgetOverview } from "./_components/travel-budget-overview";
...
interface Props {
  ...
  /** Viewer role — gates the overview tab and header actions. */
  role: string;
  /** Travel document_tracking rows for the ติดตามเอกสาร tab. */
...
export function TravelDashboardClient({
  initialRequests,
  fiscalYearOptions,
  ...
  const [rows, setRows] = useState<TravelRequestRow[]>(
    initialRequests as unknown as TravelRequestRow[],
  );
  ...
  function handleFyChange(fy: number) {
    ...
    startTransition(async () => {
      try {
        const data = await getTravelRequestsForFiscalYear(fy);
        setRows(data as unknown as TravelRequestRow[]);
      } catch {
        toast.error("ดึงข้อมูลการเดินทางไม่สำเร็จ");
      }
    });
  }
  ...
  function handleRefresh() {
    startTransition(async () => {
      try {
        const data = await getTravelRequestsForFiscalYear(selectedFy);
        setRows(data as unknown as TravelRequestRow[]);
  ...
  function handleExport() {
    startExport(async () => {
      try {
        ...
        downloadCsv(
          "travel-requests.csv",
          ["ชื่อ-สกุล", "ประเภท", "เรื่อง", "สถานที่", "วันเริ่ม", "วันสิ้นสุด", "จำนวนวัน", "งบประมาณ", "เบิกจริง", "สถานะ", "ช่องทาง", "วันที่ยื่น"],
          data.map((r) => [
            r.name,
            r.travelType,
            r.title,
            ...
            r.estimatedBudget.toString(),
            r.actualBudget.toString(),
            r.status,
            r.channel,
            r.createdAt,
          ]),
    ...
    <div className="space-y-6">
      ...
      <div className="border border-border rounded-lg bg-card p-4">
        <div className="flex items-start gap-3">
          <div className="rounded-lg bg-violet-100 text-violet-600 p-2">
            <Plane className="h-5 w-5" />
          </div>
          <div>
            <h1 className="font-semibold text-lg">จัดการเดินทางราชการ</h1>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isHrAdmin && (
              <>
                <Link href="/dashboard/hr/paper-channel/travel">
      ...
      <div className="animate-fade-in">
        ...
        {tab === "all" && (
          <div className="space-y-3">
            ...
            <TravelRequestTable requests={allFiltered} canEdit={isHrAdmin} emptyText="ไม่มีคำขอเดินทางในรอบนี้" />
          </div>
        )}
        {tab === "overview" && isHrAdmin && (
          <TravelBudgetOverview requests={roundRequests} />
        )}
        {tab === "documents" && (
          <HrDocumentsClient
            documents={documents}
