...
import {
  routeTravelCancellationToDirector,
  markTravelCancellationDirectorSigned,
  routeTravelCancellationToDean,
  markTravelCancellationDeanSigned,
  sendTravelCancellationToPresident,
  completeTravelCancellation,
  rejectTravelCancellationAtStage,
...
const STAGE: Record<string, { label: string }> = {
  ...
  completed: { label: "ใบขอยกเลิก: เสร็จสิ้น (คำสั่งเดินทางถูกยกเลิก)" },
...
export function CancellationWorkflowPanel({ cancellationId, status, reason, tracking }: Props) {
  ...
  function handleReject(r?: string) {
    ...
    setRejectLevel(null);
    if (!level) return;
    run(() => rejectTravelCancellationAtStage(cancellationId, level, r ?? ""), "ปฏิเสธคำขอยกเลิกแล้ว");
  }
    ...
    <div className="border-2 border-amber-300 rounded-lg p-4 bg-amber-50/40 space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-xs text-amber-800 font-semibold">คำขอยกเลิกคำสั่งเดินทาง (workflow แยก)</p>
      ...
      {!isTerminal && (
        <div className="flex flex-wrap gap-2">
          {status === "pending" && (
            <>
              <Button onClick={() => run(() => routeTravelCancellationToDirector(cancellationId), "ส่งให้ผอ.สำนักงานแล้ว")} disabled={isPending}>
          ...
          {status === "awaiting_director" && (
            <>
              {!directorSigned ? (
                <Button onClick={() => run(() => markTravelCancellationDirectorSigned(cancellationId), "บันทึกผอ.สำนักงานเซ็นแล้ว")} disabled={isPending}>
                  <PenLine className="h-4 w-4 mr-2" /> บันทึก: ผอ.สำนักงานลงนาม
                </Button>
              ) : (
                <Button onClick={() => run(() => routeTravelCancellationToDean(cancellationId), "ส่งให้คณบดีแล้ว")} disabled={isPending}>
          ...
          {status === "awaiting_dean" && (
            <>
              <Button onClick={() => run(() => markTravelCancellationDeanSigned(cancellationId), "คณบดีลงนามแล้ว")} disabled={isPending}>
          ...
          {status === "approved" && (
            <Button onClick={() => run(() => sendTravelCancellationToPresident(cancellationId), "ส่งอธิการบดีแล้ว")} disabled={isPending}>
          ...
          {status === "awaiting_university" && (
            <>
              <Button onClick={() => run(() => completeTravelCancellation(cancellationId), "ยกเลิกคำสั่งเดินทางแล้ว")} disabled={isPending}>
                <CheckCircle2 className="h-4 w-4 mr-2" /> บันทึก: อธิการบดีรับทราบ (ยกเลิกคำสั่ง)
              </Button>
      ...
      <ConfirmDialog
        ...
        description="ระบุเหตุผลการปฏิเสธ — คำสั่งเดินทางเดิมจะคงสถานะเสร็จสิ้นไว้"
        confirmLabel="ปฏิเสธ"
        variant="destructive"
        withInput
        inputLabel="เหตุผล"
