...
import {
  routeToTravelDirector,
  markTravelDirectorSigned,
  routeToTravelDean,
  markTravelDeanSigned,
  markTravelScanned,
  sendTravelToUniversity,
  receiveTravelFromUniversity,
  completeTravelAtFaculty,
  rejectTravelAtStage,
...
export function TravelWorkflowPanel({ requestId, status, tracking }: Props) {
  ...
  function handleReject(reason?: string) {
    ...
    setRejectLevel(null);
    if (!level) return;
    run(() => rejectTravelAtStage(requestId, level, reason ?? ""), "ปฏิเสธคำขอแล้ว");
  }
    ...
    <div className="border rounded-lg p-4 bg-card space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {status === "awaiting_dean" && (
          <>
            <Button onClick={() => run(() => markTravelDeanSigned(requestId), "คณบดีลงนาม — อนุมัติแล้ว")} disabled={isPending}>
        ...
        {status === "approved" && (
          <>
            <Button variant="outline" onClick={() => setShowScanUpload((v) => !v)} disabled={isPending}>
              <ScanLine className="h-4 w-4 mr-2" /> สแกน+อัปโหลดคำสั่ง (คณะ)
            </Button>
            <Button onClick={() => run(() => sendTravelToUniversity(requestId), "ส่งมหาวิทยาลัยแล้ว")} disabled={isPending}>
              <Building2 className="h-4 w-4 mr-2" /> ส่งมหาวิทยาลัย/อธิการบดี
            </Button>
            <Button variant="outline" onClick={() => run(() => completeTravelAtFaculty(requestId), "เสร็จสิ้น (ระดับคณะ)")} disabled={isPending}>
      ...
      {showScanUpload && status === "approved" && (
        <div className="rounded-md border border-dashed p-3">
          <p className="text-xs text-muted-foreground mb-2">อัปโหลดคำสั่งเดินทางที่ลงนามครบระดับคณะ</p>
          <FileUpload
            pathPrefix={`travel/${requestId}/faculty-signed`}
            onUploaded={(path) => run(() => markTravelScanned(requestId, path), "บันทึกเอกสารแล้ว")}
      ...
      {showReceiveUpload && status === "awaiting_university" && (
        <div className="rounded-md border border-dashed p-3">
          ...
          <FileUpload
            pathPrefix={`travel/${requestId}/university-signed`}
            onUploaded={(path) => run(() => receiveTravelFromUniversity(requestId, path), "รับเอกสารคืน — เสร็จสิ้น")}
            label="คลิกหรือลากไฟล์ (PDF/รูป)"
          />
        </div>
      )}
      ...
      <ConfirmDialog
        ...
        description="ระบุเหตุผลการปฏิเสธ — ระบบจะแจ้งพนักงาน"
        confirmLabel="ปฏิเสธ"
        variant="destructive"
        withInput
        inputLabel="เหตุผล"
