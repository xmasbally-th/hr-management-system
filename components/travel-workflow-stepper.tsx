...
/**
 * Horizontal progress stepper for a travel request's signature workflow.
 * Pure/presentational (safe in a server component). Travel has no chair stage,
 * so the node path is fixed: submit → ผอ.สำนักงาน → คณบดี → อธิการบดี → เสร็จสิ้น.
 */

type NodeKey = "submit" | "director" | "dean" | "university" | "done";
...
const NODES: NodeKey[] = ["submit", "director", "dean", "university", "done"];

/** Which node the request is currently waiting on (null = terminal). */
function currentNode(status: string): NodeKey | null {
  switch (status) {
    case "pending":
...
export function TravelWorkflowStepper({ status }: { status: string }) {
